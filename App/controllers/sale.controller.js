...
exports.findAllByDateSummary = async (req, res) => {
  ...
  var data = "";
  //customerId==="0" ? 
  data = await db.sequelize.query(`select "name",items.quantity,sum("saleDetails"."quantity") from "saleDetails","items" 
  where "saleDetails"."itemId" = items.id and   ("saleDetails"."createdAt" between '${startedDate}' and '${endDate}') 
  group by "name",items.quantity order by "name";`, {
...
exports.findAllByDateProfit = async (req, res) => {
  ...
  var queryFilter = `and ("sales"."createdAt" between '${startedDate}' and '${endDate}') `;
  ...
  if (invoiceId !== "0") {
    queryFilter = ""
    queryFilter = `and "saleDetails"."saleInvoiceId" = ${invoiceId}`;
  }
